 ...
 * 　　　　　┗┻┛　┗┻┛
 * <p>
 * Created by wangzhenxiong on 2019/3/9.
 */
public class CRC8Util {
  ...
  byte和校验
   */
    public static int byteCheckSum(byte[] byte_1) {
        int checksum = 0;
    ...
    public static int byteCheckSum(byte[] byte_1,int effective, int start,int end) {

        int checksum = 0;
        for (int i = start; i < effective-end-1; i++) {
            checksum+=byte_1[i]&0xff;
        }
        return checksum%256;
    }
